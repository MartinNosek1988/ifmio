...
type StatusTab = 'PUBLISHED' | 'PENDING' | 'DRAFT' | 'ARCHIVED'

const STATUS_LABELS: Record<StatusTab, string> = {
  PUBLISHED: 'Publikovane',
  PENDING: 'Ke schvaleni',
  DRAFT: 'Koncepty',
  ARCHIVED: 'Archiv',
}
...
export default function BoardMessagesTab({ propertyId }: { propertyId: string }) {
  ...
  const reviewMut = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Zprava posouzena')
      setRejectingId(null)
      setRejectionNote('')
    },
  })
  ...
  const publishMut = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['board-messages', propertyId] })
      toast.success('Zprava publikovana')
    },
  })
  ...
  const archiveMut = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['board-messages', propertyId] })
      toast.success('Zprava archivovana')
    },
  })
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['board-messages', propertyId] })
      toast.success('Zprava smazana')
    },
  })

  // Create form state
  const [form, setForm] = useState({ title: '', body: '', visibility: 'ALL_RESIDENTS', tags: '' as string, isPinned: false })
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['board-messages', propertyId] })
      toast.success('Zprava vytvorena')
      setShowCreate(false)
      setForm({ title: '', body: '', visibility: 'ALL_RESIDENTS', tags: '', isPinned: false })
    },
  })
  ...
  const columns: Column<BoardMessage>[] = [
    ...
    { key: 'title', label: 'Nazev', render: (row) => <span style={{ fontWeight: 500 }}>{row.title}</span> },
    ...
    {
      key: 'reads', label: 'Precteno', align: 'center' as const,
      render: (row) => (
    ...
    ...(activeTab === 'PENDING' ? [{
      ...
      render: (row: BoardMessage) => (
        <div style={{ display: 'flex', gap: 6 }}>
          <Button size="sm" variant="primary" onClick={(e: React.MouseEvent) => { e.stopPropagation(); reviewMut.mutate({ id: row.id, decision: 'APPROVED' }); }}>
            Schvalit
          </Button>
  ...
  return (
    <div data-testid="board-messages-tab">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h2 style={{ fontSize: '1rem', fontWeight: 600, margin: 0 }}>Nastenka</h2>
      ...
      {isLoading ? (
        ...
        <EmptyState
          title="Zadne zpravy"
          description={activeTab === 'PUBLISHED' ? 'Zatim nebyly publikovany zadne zpravy.' : undefined}
        />
      ) : (
        <Table
          data={messages}
        ...
        return (
          <div style={{
            ...
            <div style={{ fontSize: '.78rem', color: 'var(--text-muted)' }}>
              Autor: {msg.author?.name ?? '-'} | Vytvoreno: {new Date(msg.createdAt).toLocaleDateString('cs-CZ')}
      ...
      {/* Rejection modal */}
      <Modal open={!!rejectingId} onClose={() => { setRejectingId(null); setRejectionNote(''); }} title="Zamitnout zpravu">
        <FormField label="Duvod zamitnuti" name="rejectionNote">
          <textarea
            id="rejectionNote"
            className="input"
            value={rejectionNote}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
          <Button onClick={() => { setRejectingId(null); setRejectionNote(''); }}>Zrusit</Button>
      ...
      {/* Create modal */}
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Nova zprava na nastenku">
        <FormField label="Nazev" name="create-title">
          <input id="create-title" className="input" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
        </FormField>
        <FormField label="Text zpravy" name="create-body">
        ...
        <FormField label="Viditelnost" name="create-visibility">
          <select id="create-visibility" className="input" value={form.visibility} onChange={e => setForm({ ...form, visibility: e.target.value })}>
            <option value="ALL_RESIDENTS">Vsichni obyvatele</option>
            <option value="OWNERS_ONLY">Pouze vlastnici</option>
            <option value="TENANTS_ONLY">Pouze najemnici</option>
            <option value="BOARD_ONLY">Pouze vybor</option>
          </select>
        </FormField>
        <FormField label="Stitky" name="create-tags" required={false}>
