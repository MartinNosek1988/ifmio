...
export function PurchaseOrderForm({ open, onClose, onSuccess, editData }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.supplierName) errs.supplierName = 'Vyplňte dodavatele';
    if (items.length === 0 || !items[0].description) errs.items = 'Přidejte alespoň jednu položku';
    ...
    try {
      ...
      setErrors({ _form: 'Nepodařilo se uložit objednávku' });
    }
  };
        ...
        <table className="table" style={{ width: '100%', marginBottom: 8 }}>
          <thead>
            <tr>
              ...
              <th style={{ width: '15%', textAlign: 'right' }}>Celkem</th>
              <th style={{ width: '8%' }}></th>
            </tr>
          </thead>
          <tbody>
      ...
      <FormSection title="Souhrn">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 12, fontSize: '0.92rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: 'var(--text-muted)' }}>Základ:</span>
            <span style={{ fontWeight: 500 }}>{formatKc(amountBase)}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: 'var(--text-muted)' }}>DPH ({vatRate}%):</span>
