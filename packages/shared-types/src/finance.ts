...
export type CreateTransactionDto = Pick<
  FinanceTransaction,
  'type' | 'amount' | 'vatRate' | 'description' | 'date' | 'propertyId'
>;
...
export type CreateBankAccountDto = Pick<
  BankAccount, 'name' | 'accountNumber' | 'propertyId'
> & Partial<Pick<BankAccount, 'iban' | 'bankCode' | 'currency'>>;

export type CreatePrescriptionDto = Pick<
  Prescription,
  'propertyId' | 'type' | 'amount' | 'description' | 'validFrom'
> & Partial<Pick<
  Prescription,
  'unitId' | 'residentId' | 'vatRate' | 'dueDay' | 'variableSymbol' | 'validTo'
